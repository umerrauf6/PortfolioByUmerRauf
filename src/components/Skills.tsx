...
// ─── Game 3D gem for each skill card ─────────────────────────────────
function SkillGem({ color, hovered }: { color: string; hovered: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (outer.current) {
      outer.current.rotation.y = t * (hovered ? 0.9 : 0.35);
      outer.current.rotation.x = t * (hovered ? 0.4 : 0.15);
    }
    if (inner.current) {
  ...
  return (
    <group scale={hovered ? 1.12 : 1}>
      ...
      <mesh ref={outer}>
        ...
        <meshBasicMaterial color={threeColor} wireframe transparent opacity={hovered ? 0.6 : 0.35} />
      </mesh>
      {/* Inner solid core */}
      <mesh ref={inner}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color={threeColor} transparent opacity={hovered ? 0.06 : 0.02} />
      </mesh>
    </group>
  );
}
    ...
    >
      <div className="section-inner" ref={ref}>
        ...
        >
          ...
          <p className="text-sm" style={{ color: 'rgba(120,180,210,0.6)', fontFamily: 'var(--font-mono)', maxWidth: 500 }}>
            // Full-stack toolkit refined through B2B projects &amp; academic research
          </p>
        </motion.div>
          ...
          {skillGroups.map((group, i) => {
            ...
            return (
              <motion.div
                ...
                className="shine-effect bracket-corners bracket-corners-inner relative"
                style={{
                ...
                <div className="relative h-36">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <span
                      ...
                      style={{
                        color: group.color,
                        textShadow: `0 0 20px ${group.color}`,
                      }}
                    >
                      {group.category}
                    </span>
                    ...
                    <div key={name}>
                      ...
                      <div className="h-px rounded-full overflow-hidden relative" style={{ background: 'rgba(255,255,255,0.06)' }}>
                        <motion.div
                          ...
                          transition={{ duration: 0.9, delay: i * 0.1 + 0.3, ease: 'easeOut' as const }}
                        />
                      </div>
                    </div>
                  ))}
        ...
        >
          {['React','Next.js','TypeScript','JavaScript','Node.js','NestJS','MongoDB','REST APIs','Tailwind CSS','Git','Agile','LLM APIs','GitHub Copilot','Claude Code'].map(tag => (
            <motion.span key={tag} className="tech-tag" whileHover={{ scale: 1.07, y: -2 }}>
              {tag}
            </motion.span>
          ))}
        </motion.div>
